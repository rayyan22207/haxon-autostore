...
<template>
  <footer v-if="!isAdmin" class="border-t border-white/10 bg-[#050608] text-white"><div class="mx-auto max-w-7xl px-5 py-12"><div class="grid gap-8 md:grid-cols-[1.2fr_.8fr]"><div><p class="text-3xl font-black tracking-[0.22em]">{{ content.siteSettings.brandName }}</p><p class="mt-3 max-w-xl text-sm leading-6 text-white/50">{{ content.siteSettings.footerStatement }}</p></div><div class="flex flex-wrap gap-4 md:justify-end"><router-link v-for="link in links" :key="link.label" :to="link.to" class="text-sm text-white/55 hover:text-white">{{ link.label }}</router-link><router-link v-if="auth.isAdmin" to="/admin" class="text-sm text-red-300 hover:text-white">Admin</router-link></div></div><p class="mt-8 text-xs text-white/35">© {{ year }} {{ content.siteSettings.brandName }}. {{ content.siteSettings.location }}</p></div></footer>
