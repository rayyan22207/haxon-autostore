<script setup>
import { computed, onMounted, ref } from 'vue'
import { useProductStore } from '../../stores/productStore'

const productStore = useProductStore()
const track = ref(null)

const products = computed(() => {
  const featured = productStore.products.filter((p) => p.featured || p.isFeatured)
  return (featured.length ? featured : productStore.products).slice(0, 12)
})

const formatPrice = (price) => {
  if (!price) return 'Price on request'

  return `PKR ${Number(price).toLocaleString()}`
}

const productImage = (product) => {
  return product.image || product.imageUrl || product.images?.[0] || '/images/products/placeholder.png'
}

onMounted(() => productStore.loadProducts?.())
</script>

<template>
  <section class="relative overflow-hidden bg-white py-8 text-black sm:py-10">
    <div class="mx-auto max-w-[1500px] px-5 sm:px-8 lg:px-12">
      <div class="mb-5 flex items-center justify-between gap-4">
        <div>
          <p class="text-[10px] font-black uppercase tracking-[0.28em] text-[#E50914]">
            Featured products
          </p>

          <h2 class="mt-2 text-2xl font-black uppercase tracking-[-0.03em] text-black sm:text-3xl">
            The Haxon Selection.
          </h2>
        </div>

        <router-link
          to="/products"
          class="group hidden items-center gap-3 text-[10px] font-black uppercase tracking-[0.18em] text-black/45 transition hover:text-black sm:inline-flex"
        >
          View All Products
          <span class="text-[#E50914] transition group-hover:translate-x-1">→</span>
        </router-link>
      </div>

      <div class="relative">
        <div
          ref="track"
          class="featured-products-track flex gap-2 overflow-x-auto scroll-smooth pb-2 lg:gap-2"
        >
          <router-link
            v-for="product in products"
            :key="product.id"
            :to="`/products/${product.slug || product.id}`"
            class="group relative min-h-[330px] min-w-[285px] overflow-hidden border border-white/10 bg-[#08090b] px-7 py-6 transition-all duration-300 hover:-translate-y-1 hover:border-white/20 hover:bg-[#101114] sm:min-w-[310px] lg:min-w-[295px]"
          >
            <div class="flex h-[165px] items-center justify-center">
              <img
                :src="productImage(product)"
                :alt="product.name"
                class="max-h-[150px] w-full object-contain opacity-90 transition duration-500 group-hover:scale-105 group-hover:opacity-100"
              />
            </div>

            <div class="mt-5">
              <p
                v-if="product.brand"
                class="text-[10px] font-black uppercase tracking-[0.18em] text-white/35"
              >
                {{ product.brand }}
              </p>

              <h3 class="mt-2 line-clamp-2 text-[17px] font-semibold leading-6 text-white/82 transition group-hover:text-white">
                {{ product.name }}
              </h3>

              <p class="mt-5 text-base font-black tracking-[-0.02em] text-white">
                {{ formatPrice(product.price || product.salePrice) }}
              </p>
            </div>
          </router-link>
        </div>
      </div>
    </div>
  </section>
</template>

<style scoped>
.featured-products-track {
  scrollbar-width: none;
}

.featured-products-track::-webkit-scrollbar {
  display: none;
}
</style>